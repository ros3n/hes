...
func NewWorker(group sync.WaitGroup, callbackCh chan<- manager.SendStatus,
	mf *mailer.AbstractMailerFactory, providers ...mailer.EmailProvider) *Worker {
	...
	return &Worker{mailerFactory: mf, group: group, callbackCh: callbackCh, nextProvider: next(providers...)}
}
...
func (w *Worker) sendEmail(email *models.Email) {
	...
	if err != nil {
		...
		w.callbackCh <- manager.SendStatus{EmailID: email.ID, Success: false}
		return
	}

	log.Printf("Email %d sent successfully\n", email.ID)
	w.callbackCh <- manager.SendStatus{EmailID: email.ID, Success: true}
}
